...
import {
  ...
  ChartBarIcon, KeyIcon, WalletIcon, HomeIcon, UserCircleIcon, BellIcon, CreditCardIcon, ClipboardListIcon, DocumentTextIcon, BookOpenIcon
...
export const MANAGER_NAV: NavItem[] = [
  ...
  { label: 'Tenants', icon: UsersIcon, path: '/manager/tenants', section: 'PROFILE' },
  { label: 'Owners', icon: BriefcaseIcon, path: '/manager/owners', section: 'PROFILE' },
  ...
  { label: 'Chart of Accounts', icon: BookOpenIcon, path: '/manager/chart-of-accounts', section: 'FINANCE' },
  { label: 'Tax Center', icon: DocumentTextIcon, path: '/manager/tax-center', section: 'FINANCE' },
...
export const allMaintenanceRequests: MaintenanceRequest[] = [
    ...
    { id: 'm004', property: 'Oakwood Lofts', building: 'Main Building', unit: 'Unit 201', tenant: 'James Wilson', issue: 'No hot water', details: 'There is no hot water in the entire apartment. This is an emergency!', priority: 'Emergency', status: 'In Progress', submittedDate: new Date('2025-11-10T11:45:00Z').toISOString(), assignedTo: 'v001' },
...
export const initialDocuments: Document[] = [
    ...
    { id: 'folder3', name: 'Sunset Villas', type: 'Folder', property: 'All Properties', path: '/', uploadDate: new Date('2023-01-15T00:00:00Z').toISOString() },
    { id: 'folder4', name: 'The Grand Apartments', type: 'Folder', property: 'All Properties', path: '/', uploadDate: new Date('2023-01-15T00:00:00Z').toISOString() },
    ...
    { id: 'doc2', name: 'Q2 2024 Owner Statement - Greenleaf.pdf', type: 'Legal', property: 'All Properties', path: '/', size: '850 KB', uploadDate: new Date('2024-07-05T00:00:00Z').toISOString() },
    { id: 'doc3', name: 'Move-in Inspection - S. Nguyen.pdf', type: 'Inspection Report', property: 'The Grand Apartments', path: '/The Grand Apartments/', unit: '101', size: '2.5 MB', uploadDate: new Date('2023-07-30T00:00:00Z').toISOString() },
    { id: 'doc4', name: 'Roofer Contract.pdf', type: 'Contract', property: 'Sunset Villas', path: '/Sunset Villas/', size: '780 KB', uploadDate: new Date('2024-05-18T00:00:00Z').toISOString(), vendorId: 'v001' },
];
...
export const billingHistory: BillingHistoryItem[] = [
  ...
  { id: 'bh002', date: 'Apr 1, 2024', description: 'Pro Plan - Monthly Subscription', amount: 99.00, status: 'Paid' },
  { id: 'bh003', date: 'Mar 1, 2024', description: 'Pro Plan - Monthly Subscription', amount: 99.00, status: 'Paid' },
];

export const savedPaymentMethods: TenantPaymentMethod[] = [
    { id: 'pm1', type: 'Card', details: 'Visa ending in 1234', isPrimary: true },
    { id: 'pm2', type: 'Bank', details: 'Chase Checking ...5678', isPrimary: false },
];

export const rentRollData: RentRollItem[] = allTenants
    .filter(t => t.status === 'Active' || t.status === 'Future')
    .map(tenant => {
        const property = properties.find(p => p.name === tenant.propertyName);
        const unit = property?.buildings.flatMap(b => b.units).find(u => u.name === tenant.unitName);
        const tenantInUnit = unit?.tenants.find(t => t.name === tenant.name);

        const dueDate = new Date();
        dueDate.setDate(1);

        return {
            id: `${unit?.name}-${tenant.id}`,
            tenantName: tenant.name,
            propertyName: tenant.propertyName,
            unitName: tenant.unitName,
            rent: tenantInUnit?.rentPortion || unit?.rent || 0,
            balance: 0,
            dueDate: dueDate.toISOString(),
            status: 'Upcoming' as RentRollItem['status'],
            paidDate: null,
        };
    }).filter(item => item.rent > 0);

export const initialTransactions: Transaction[] = [
    ...rentRollData.filter(i => i.status === 'Paid').map((item, index) => ({
        id: `txn-inc-${index}`,
        date: new Date(item.paidDate || '2024-06-01').toISOString(),
        description: `Rent - ${item.propertyName}, ${item.unitName}`,
        property: item.propertyName,
        owner: properties.find(p => p.name === item.propertyName)?.owner || '',
        category: 'Income' as const,
        type: 'Rent' as const,
        amount: item.rent,
    })),
    ...allMaintenanceRequests.filter(i => i.status === 'Completed').map((item, index) => ({
        id: `txn-exp-${index}`,
        date: new Date(item.submittedDate).toISOString(),
        description: `Maintenance - ${item.issue}`,
        property: item.property,
        owner: properties.find(p => p.name === item.property)?.owner || '',
        category: 'Expense' as const,
        type: 'Maintenance' as const,
        amount: Math.floor(Math.random() * (500 - 50 + 1) + 50), // random amount
    }))
];

export const notifications: Notification[] = [
    { id: 'n1', type: 'maintenance', text: 'New maintenance request from Olivia Martinez.', timestamp: new Date('2025-11-10T09:01:00Z').toISOString(), isRead: false, link: '/manager/maintenance' },
    { id: 'n2', type: 'financial', text: 'Rent payment of $1750 received from Benjamin Carter.', timestamp: new Date('2025-11-01T10:30:00Z').toISOString(), isRead: false, link: '/manager/rent-roll' },
    { id: 'n3', type: 'lease', text: 'Lease for Isabella Garcia is expiring in 90 days.', timestamp: new Date('2025-11-01T08:00:00Z').toISOString(), isRead: true, link: '/manager/tenants' },
    { id: 'n4', type: 'message', text: 'New message from Greenleaf Investments.', timestamp: new Date('2024-06-10T18:30:00Z').toISOString(), isRead: true, link: '/manager/messages' },
];

export const initialLeaseTemplates: LeaseTemplate[] = [
  {
    id: 'ltpl_std_12m',
    name: 'Standard 12-Month Residential Lease',
    isDefault: true,
    content: `<h1>Residential Lease Agreement</h1>
<p>This Lease Agreement (the "Agreement") is made and entered on {{lease.date}} by and between {{owner.name}} ("Landlord") and {{tenant.name}} ("Tenant").</p>
<h2>1. PROPERTY</h2>
<p>Landlord, in consideration of the rent to be paid and the covenants and agreements to be performed and observed by the Tenant, does hereby lease to the Tenant the following described residential property located at: {{property.address}}, Unit {{unit.name}} (the "Premises").</p>
<h2>2. TERM</h2>
<p>The term of this Lease shall commence on {{lease.startDate}} and shall continue until its expiration on {{lease.endDate}}.</p>
<h2>3. RENT</h2>
<p>The total monthly rent for the Premises is <strong>\${{lease.rentAmount}}</strong>, due on the 1st day of each month.</p>
<h2>4. SECURITY DEPOSIT</h2>
<p>Upon execution of this Agreement, Tenant shall deposit with Landlord the sum of <strong>\${{lease.securityDeposit}}</strong> as security for the faithful performance by Tenant of the terms hereof.</p>
<p>IN WITNESS WHEREOF, the parties have executed this Lease as of the date first above written.</p>
<br/><br/>
<p>_________________________</p>
<p>Landlord: {{owner.name}}</p>
<br/><br/>
<p>_________________________</p>
...
`
  ...
  {
    id: 'ltpl_mtm_basic',
    name: 'Basic Month-to-Month Agreement',
    isDefault: false,
    content: `<h1>Month-to-Month Rental Agreement</h1>
<p>This agreement is between {{owner.name}} (Landlord) and {{tenant.name}} (Tenant) for the rental of property at {{property.address}}, Unit {{unit.name}}.</p>
<p>This agreement begins on {{lease.startDate}} and continues on a month-to-month basis. Rent of \${{lease.rentAmount}} is due on the 1st of each month. A security deposit of \${{lease.securityDeposit}} has been paid.</p>
<p>This agreement may be terminated by either party by giving 30 days written notice.</p>
`
  },
];
...
export const initialChartOfAccounts: Account[] = [
    // Assets
    { id: 'acc_1010', number: '1010', name: 'Operating Bank Account', type: 'Asset', description: 'Primary checking account for operations.', isSystemAccount: true },
    { id: 'acc_1020', number: '1020', name: 'Security Deposit Holdings', type: 'Asset', description: 'Escrow account for tenant security deposits.', isSystemAccount: true },
    { id: 'acc_1200', number: '1200', name: 'Accounts Receivable', type: 'Asset', description: 'Money owed by tenants.', isSystemAccount: true },
    // Liabilities
    { id: 'acc_2010', number: '2010', name: 'Security Deposits Liability', type: 'Liability', description: 'Tenant security deposits held.', isSystemAccount: true },
    { id: 'acc_2100', number: '2100', name: 'Accounts Payable', type: 'Liability', description: 'Money owed to vendors.', isSystemAccount: true },
    // Income
    { id: 'acc_4010', number: '4010', name: 'Rental Income', type: 'Income', description: 'Primary income from rent payments.', isSystemAccount: true },
    { id: 'acc_4020', number: '4020', name: 'Late Fee Income', type: 'Income', description: 'Income from tenant late fees.' },
    { id: 'acc_4030', number: '4030', name: 'Parking Income', type: 'Income', description: 'Income from parking fees.' },
    { id: 'acc_4900', number: '4900', name: 'Other Income', type: 'Income', description: 'Miscellaneous income sources.' },
    // Expenses
    { id: 'acc_6050', number: '6050', name: 'Repairs & Maintenance', type: 'Expense', description: 'Costs for general repairs and maintenance.' },
    { id: 'acc_6100', number: '6100', name: 'Utilities', type: 'Expense', description: 'Utility costs for common areas or vacant units.' },
    { id: 'acc_6150', number: '6150', name: 'Management Fees', type: 'Expense', description: 'Fees paid to the management company.' },
    { id: 'acc_6200', number: '6200', name: 'Property Taxes', type: 'Expense', description: 'Taxes paid on properties.' },
    { id: 'acc_6250', number: '6250', name: 'Insurance', type: 'Expense', description: 'Property insurance premiums.' },
    { id: 'acc_6900', number: '6900', name: 'Miscellaneous Expense', type: 'Expense', description: 'Uncategorized expenses.' },
];
