...
const TenantScreening: React.FC = () => {
    ...
    const { tenants, updateTenantScreening } = useData();
    ...
    return (
        <DashboardLayout navItems={MANAGER_NAV} activePath="/manager/tenants">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 mb-8 flex justify-between items-center">
                ...
                {screening.overallStatus === 'Awaiting Decision' || screening.overallStatus === 'In Progress' ? (
                    ...
                    <button onClick={() => alert('Navigate to lease creation')} className="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700">Create Lease</button>
                ): null }
            </div>
