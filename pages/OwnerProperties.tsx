...
import { DollarIcon, HomeIcon, WrenchIcon, UsersIcon, DocumentIcon, MapPinIcon, XMarkIcon, MapIcon, ListBulletIcon } from '../components/Icons';
...
const PropertyDetailView: React.FC<{ property: Property & { totalUnits: number; occupiedUnits: number; monthlyRevenue: number; openMaintenance: number; } }> = ({ property }) => {
        ...
        switch (activeTab) {
            case 'Overview': return (
                <div className="space-y-6">
                    ...
                    <div>
                         <h4 className="text-md font-semibold text-slate-700 mb-3">Recent Maintenance</h4>
            ...
            case 'Units': return (
                 <table className="w-full text-left">
                    ...
                    <tbody>
                        {property.buildings.flatMap(b => b.units).map(unit => {
    ...
    const tabs = ['Overview', 'Units', 'Financials', 'Documents'];
        ...
        <>
            ...
            <nav className="border-b px-6">
                <div className="flex space-x-6">
                    {tabs.map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`py-3 text-sm font-semibold border-b-2 ${activeTab === tab ? 'border-blue-500 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>
                            {tab}
                        </button>
                    ))}
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b">
                            <tr className="text-xs text-slate-500 uppercase font-semibold">
                                ...
                                <th className="px-6 py-3">Open Maintenance</th>
                            </tr>
                        </thead>
                            ...
                            {filteredProperties.map(prop => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-sm font-medium text-slate-800">{formatCurrency(prop.monthlyRevenue)}</td>
